...
Current Slots: {{"height": null, "weight": null, "fitness_program": null, "avoid_eating": null}}
Last line:
Person #1: 180 CM
Output Slots: {{"height": "180 CM", "weight": null, "fitness_program": null, "avoid_eating": null}}
END OF EXAMPLE
...
Current Slots: {{"height": "183 CM", "weight": null, "fitness_program": null, "avoid_eating": null}}
Last line:
Person #1: 75 KG
Output Slots: {{"height": "183 CM", "weight": "75 KG", "fitness_program": null, "avoid_eating": null}}
END OF EXAMPLE
...
# need to modify
def generate_dynamic_prompt(current_slots, history):
    questions = {
        "height": "May I know your height?",
        "weight": "What's your weight?",
        "fitness_program": "Which fitness program are you interested in? (Losing weight, gaining muscle, etc.)",
        "avoid_eating": "Is there anything you avoid eating? (Allergies, no meat, no sugar, etc.)",
    }

    dynamic_questions = ""
    for slot, question in questions.items():
        if current_slots.get(slot, "null") == "null":
            dynamic_questions += question + " "

    prompt_template = _DEFAULT_DIET_PLAN_TEMPLATE.format(
        finish="False",
        history=history,
        slots=json.dumps(current_slots),
        input=dynamic_questions
    )
